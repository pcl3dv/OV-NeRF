...
class Evaluator(object):
    ...
    def _generate_matrix(self, gt_image, pre_image, ignore_labels):
        ...
        for IgLabel in ignore_labels:
            mask &= (gt_image != IgLabel)        # remove classes from unlabeled pixels in gt image and predict
    ...
    def add_batch(self, gt_image, pre_image, ignore_labels):
        assert gt_image.shape == pre_image.shape
        self.confusion_matrix += self._generate_matrix(gt_image, pre_image, ignore_labels)
...
def measure_pa_miou(num_class, gt_image, pre_image):
    ...
    ignore_labels = [0]
    metric.add_batch(gt_image, pre_image, ignore_labels)
    
    acc = metric.Pixel_Accuracy()
    mIoU = metric.Mean_Intersection_over_Union()
    
    return acc*100, mIoU*100
